...
** GDB
Changing the colors of the gdb cursor/mark/line...
#+begin_src emacs-lisp
  (require 'nadvice)
...
#+end_src
** Moving trough paragraphs
For some reason moving forward and backward on paragraphs is bound to
*C-<down>*. Since I'm not a big fan of arrows...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-}") 'forward-paragraph)
  (global-set-key (kbd "C-{") 'backward-paragraph)
#+end_src
** Moving trough windows
#+begin_src emacs-lisp
  (defun prev-window ()
    (interactive)
    (other-window -1))
  ...
  (global-set-key (kbd "C-ç") #'prev-window)
#+end_src
...
**** SDF vterm patch
For some reason it was quite difficult to open vterm trough a
tramp/dired connection to the sdf server... This for sure due to my
lack of knowledge... This patch allows vterm to "just work" on the sdf server.
#+begin_src emacs-lisp
  ...
  (advice-add 'vterm :around #'my-vterm-bsd-compatibility)
#+end_src

**** Vterm and emacs interoperability 
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    :defer t
    :preface
    ;; This function is from Manuel Uberti
    ;; https://manueluberti.eu/posts/2018-02-17-magit-bury-buffer/
...
** Remember mode
#+begin_src emacs-lisp
  (setq remember-data-file "~/Org/Notes/notes")
#+end_src
  ...
  (use-package org
    ...
    (setq org-structure-template-alist
  	  '(("el"  . "src emacs-lisp")
  	    ...
  	    ("cpp" . "src cpp :results output :exports both")
  	    ("ft"  . "src fortran")
  	    ("cl"  . "src lisp")
...
Python mode for python files
   ...
   #+begin_src emacs-lisp
     (use-package pyvenv
       :ensure t
       :init
       (setenv "WORKON_HOME" (expand-file-name "~/py_home")))
   #+end_src
   ...
   #+begin_src emacs-lisp
     ...
     (setq ein:output-area-inlined-image-properties '())
   #+end_src
...
** Emacs Lisp
#+begin_src emacs-lisp
  (defun eval-and-insert-arrow ()
    "Evaluate the preceding sexp and
  insert its result prefixed with ⇒."
    (interactive)
